...
public class GUIElementRenderer
{
    ...
    private static final int WELL_HIGHLIGHT_X = 26;
    private static final int WELL_HIGHLIGHT_Y = 18;
    private static final int WELL_HIGHLIGHT_W = 22;
    private static final int WELL_HIGHLIGHT_H = 22;
    ...
    public void drawSlotHighlight(GuiGraphics gui, int x, int y)
    {
        gui.blit(GUI, x - 3, y - 3, WELL_HIGHLIGHT_X, WELL_HIGHLIGHT_Y, WELL_HIGHLIGHT_W, WELL_HIGHLIGHT_H, TEXTURE_WIDTH, TEXTURE_HEIGHT);
    }
...
//    }
    ...
    public void drawLeftArrow(GuiGraphics gui, int x, int y, float scale)
    {
        preDraw(gui, x, y, ARROW_LEFT_W, ARROW_LEFT_H,  scale);
        gui.blit(GUI, 0, 0, ARROW_LEFT_X, ARROW_LEFT_Y, ARROW_LEFT_W, ARROW_LEFT_H, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        postDraw(gui);
    }

    public void drawRightArrow(GuiGraphics gui, int x, int y,  float scale)
    {
        preDraw(gui, x, y, ARROW_RIGHT_W, ARROW_RIGHT_H, scale);
        gui.blit(GUI, 0, 0, ARROW_RIGHT_X, ARROW_RIGHT_Y, ARROW_RIGHT_W, ARROW_RIGHT_H, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        postDraw(gui);
    }

    public void drawUpArrow(GuiGraphics gui, int x, int y, float scale)
    {
        preDraw(gui, x, y, ARROW_UP_W, ARROW_UP_H, scale);
        gui.blit(GUI, 0, 0, ARROW_UP_X, ARROW_UP_Y, ARROW_UP_W, ARROW_UP_H, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        postDraw(gui);
    }

    public void drawDownArrow(GuiGraphics gui, int x, int y,  float scale)
    {
        preDraw(gui, x, y, ARROW_DOWN_W, ARROW_DOWN_H, scale);
    ...
    private void preDraw(GuiGraphics gui, int x, int y, int w, int h, float scale)
    {
        gui.pose().pushPose();
